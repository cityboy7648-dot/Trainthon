export const copy = {
  ...
  dashboard: {
    ...
    completionBasis: "이미지·캡션 생성 완료 기준",
    emptyProgress: "진행 중인 캠페인이 없어요.",
    taskTitle: (day: number, title: string) => `D${day} · ${title}`,
    taskProgress: (done: number, total: number) => `${done}/${total}개 작업 완료`,
    viewImage: (title: string) => `${title} 사진 크게 보기`,
    refresh: "현황 새로고침",
    refreshing: "현황을 불러오고 있어요",
    summary: "브랜드와 캠페인 요약",
    myBrand: "내 브랜드",
  ...
  campaigns: {
    ...
    realUsage: {
      ...
      description: "선택한 상품으로 5일치 피드 이미지와 캡션을 만들어요.",
      emptyTitle: "생성할 상품이 없어요",
      emptyDescription: "브랜드에서 상품 이름과 이미지를 먼저 저장해 주세요.",
      brandLink: "브랜드 확인",
      submit: "5일 캠페인 만들기",
      pending: "장면과 캡션을 기획하고 있어요.",
      ready: "이미지 5장과 캡션이 완성됐어요",
      planReady: "5일 기획이 저장됐어요",
      planOnly: "이미지를 생성하기 전에 저장한 기획안입니다.",
      direction: "촬영 방향",
      scene: "장면",
      caption: "캡션",
      generating: "같은 상품과 인물로 이미지 5장을 만들고 있어요.",
      legacyPlan: "기획만 저장된 캠페인이에요. 새 캠페인으로 이미지 생성을 시작해 주세요.",
      retry: "새 캠페인으로 다시 만들기",
    },
    title: "캠페인",
    galleryTitle: "최근 캠페인",
    newCampaign: "새 캠페인 만들기",
  ...
  campaignGallery: {
    ...
    to: "종료일",
    dateHint: "캠페인 마감일 기준",
    invalidRange: "종료일을 시작일 이후로 선택해 주세요.",
    sort: "정렬",
    newest: "최신순",
    deadline: "마감일순",
    name: "이름순",
    ...
    allChannels: "전체 채널",
    reset: "필터 초기화",
    noResults: "조건에 맞는 캠페인이 없어요",
    unknownChannel: "채널 미정",
    unknownDate: "일정 미정",
    preparing: "이미지 준비 중",
    ...
    endDate: (value: string) =>
      `${Number(value.slice(5, 7))}월 ${Number(value.slice(8, 10))}일까지`,
  },
  campaignWorkspace: {
