...
코딩 에이전트용. 제품이 뭔지는 `README.md`. 어떻게 만드는지는 `docs/PLAN.md`.
...
- 스펙에 없는 기능을 추가하지 않는다.
- Figma를 고칠 때 V1·V2 같은 버전 프레임을 만들지 않는다. 기존 프레임을 지우고 그 자리에 넣는다.
...
- 테이블은 `docs/PLAN.md`의 "Supabase 테이블"에 있는 것만. 그 표가 스키마의 기준이다.
- 새 테이블·컬럼이 필요하면 코드 전에 `docs/PLAN.md`를 고치는 PR을 먼저. 왜 필요한지 적는다.
